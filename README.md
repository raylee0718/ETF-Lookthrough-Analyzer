...
「備份資料」可下載完整 JSON 備份，內容包含交易紀錄、手動持股、價格資料、ETF 成分股與使用設定。還原時可選擇檔案或貼上 JSON，系統會先驗證與顯示交易、價格、ETF 成分股組數與備份時間，確認後才會取代目前資料；無效或不相容的備份不會套用。
...
「交易紀錄」頁可貼上多筆簡單 CSV-like 交易資料，先預覽有效列與錯誤列，確認後只追加有效交易到既有紀錄。支援欄位順序為：日期、代號、名稱、類別、買賣、股數、成交價、手續費、交易稅、備註；日期必須是 `YYYY-MM-DD`，買賣支援 `買進` / `賣出`，手續費與交易稅空白會視為 0。此功能只新增貼上與預覽流程，不解析券商對帳單、不做 OCR、不連接券商，也不改交易計算、價格更新、ETF 成分股更新或穿透分析邏輯。
